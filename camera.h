...
/*
  ...
  */
  ...
  s->set_gain_ctrl(s, 1);                 // Desactiva auto gain
  s->set_agc_gain(s, 0);                  // Ajuste manual  (ANTES1)
  s->set_exposure_ctrl(s, 1);             // Desactiva auto exposure
  s->set_aec_value(s, 300);               // Prueba con este valor y ajusta
  s->set_whitebal(s, 1);                // Desactiva auto balance de blancos
  s->set_awb_gain(s, 1);                // Desactiva AWB gain
